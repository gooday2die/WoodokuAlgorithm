...
SMALLTYPE Field::calculateScore(void){
    ...
    for (SMALLTYPE i = 0 ; i < 3 ; i++){
        for(SMALLTYPE j = 0 ; j < 3 ; j++) {
            streaks += (pixels[j] >> (9 * (2 - i)) & 0x1FF) == 0x1FF;
    ...
    for(SMALLTYPE i = 0 ; i < 9 ; i++){
        ...
        streaks += (curCol[0] == curCol[1]) && (curCol[1] == curCol[2]); // this counts 0s as well
        streaks -= (curCol[0] == 0); // remove 0s.
    ...
    for (SMALLTYPE i = 0 ; i < 3 ; i++){
        ...
        blk_array[i] &= (pixels[i] ^ 0x70381C0); // do XOR
        blk_array[i] &= (pixels[i] ^ 0xe07038);
        blk_array[i] &= (pixels[i] ^ 0x1c0e07);
    }
    ...
    score = streaks * 18 + 10 * (streaks > 0); // calculate score
